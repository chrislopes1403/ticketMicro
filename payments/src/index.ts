...
const start = async () => {
  ...
  if(!process.env.PAYMENTS_MONGO_URL)
  {
  ...
  {
      throw new Error('STRIPE_SECRET is undefined');   
  }
  try
  {
    ...
    await mongoose.connect(process.env.PAYMENTS_MONGO_URL!, {
  ...
  app.listen(3000, () => {
    console.log('Payments server on port 3000!!!!!!!!');
  });
};
